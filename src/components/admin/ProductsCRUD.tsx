...
export default function ProductsCRUD() {
  ...
  // Raw string states so the user can type freely (e.g. "9." mid-input)
  ...
  async function handleSubmit() {
    ...
    const payload = {
      slug: form.slug, nameEs: form.nameEs,
      shortDescEs: form.shortDescEs,
    ...
    try {
      ...
      closeModal();
      await loadProducts(); // re-fetch to get fresh data (includes images)
    } catch (e: unknown) {
      ...
      {modalOpen && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-2xl bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      onBlur={() => { if (!editing && !form.slug) setForm((f) => ({ ...f, slug: toSlug(f.nameEs) })); }}
                      className={inputClass}
                    />
                    {errors.nameEs && <p className="text-red-400 text-xs mt-1">{errors.nameEs}</p>}
                  </div>
                  <div>
                    <label className="block text-xs text-[var(--text-muted)] mb-1">Descripción corta</label>
                    <input
                      value={form.shortDescEs}
                      onChange={(e) => setForm({ ...form, shortDescEs: e.target.value })}
                      className={inputClass}
                    />
                    {errors.shortDescEs && <p className="text-red-400 text-xs mt-1">{errors.shortDescEs}</p>}
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (val === '' || /^\d*\.?\d*$/.test(val)) {
                          setPriceInput(val);
                          setForm((f) => ({ ...f, price: parseFloat(val) || 0 }));
                        }
                      }}
                      className={inputClass}
                    />
